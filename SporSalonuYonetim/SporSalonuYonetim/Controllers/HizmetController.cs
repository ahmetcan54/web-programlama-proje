    ...
    public class HizmetController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("Ad,Aciklama,Sure,Ucret,Kategori,Aktif,SporSalonuId,AntrenorId")] Hizmet hizmet)
        {
            if (ModelState.IsValid)
            {
                ...
                _logger.LogInformation("Yeni hizmet oluþturuldu: {Ad}", hizmet.Ad);
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            if (hizmet.Randevular != null && hizmet.Randevular.Any())
            {
                hizmet.Aktif = false;
                _context.Update(hizmet);
                TempData["InfoMessage"] = "Hizmet randevularý olduðu için devre dýþý býrakýldý.";
            }
            else
            {
            ...
            await _context.SaveChangesAsync();
            _logger.LogInformation("Hizmet silindi/devre dýþý býrakýldý: {Id}", id);
